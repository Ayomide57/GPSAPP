...
async function register(navigation:any):Promise<TransistorAuthorizationToken> {
  ...
  await BackgroundGeolocation.destroyTransistorAuthorizationToken('https://tracker.transistorsoft.com');
  ...
  let token:TransistorAuthorizationToken = await BackgroundGeolocation.findOrCreateTransistorAuthorizationToken(orgname, username, 'https://tracker.transistorsoft.com');
